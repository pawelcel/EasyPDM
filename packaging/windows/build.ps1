...
# packaging\windows\publish\ — uruchom PRZED kompilacją EasyPDM.iss w Inno Setup Compiler.
...
Push-Location (Join-Path $RepoRoot "EasyPDM.Web")
try {
    npm ci
...
dotnet publish (Join-Path $RepoRoot "EasyPDM.Api") -c Release -r win-x64 --self-contained true `
...
Write-Host "Teraz skompiluj packaging\windows\EasyPDM.iss w Inno Setup Compiler (iscc.exe),"
Write-Host "np.: iscc packaging\windows\EasyPDM.iss"
