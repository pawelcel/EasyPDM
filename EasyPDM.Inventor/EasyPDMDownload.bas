...
' ============================================================================
' EasyPDMDownload -- Autodesk Inventor macro that downloads a Part/Assembly from EasyPDM and
' opens it.
'
' This is the INVENTOR PORT of EasyPDM.SolidWorks/EasyPDMDownload.bas -- see that file's own
' header, and EasyPDM.Inventor/EasyPDMUpload.bas's header, for the general reasons behind
' this macro family's design choices (own minimal JSON, no UserForm anywhere -- VBA
' UserForms are separate binary resources, not embeddable in a plain .bas text file).
'
...
' than importing them, the same way the SolidWorks and FreeCAD counterparts of this file do.
' The API address, session token and download folder are shared with
' EasyPDM.Inventor/EasyPDMUpload.bas (and, deliberately, with the SolidWorks/FreeCAD macro
' families too) through the SAME Windows registry keys -- logging in from any of them is
' enough for all.
'
...
'      the Part/Assembly. The macro waits (WaitForTicket, polling GET /create-tickets/
'      {ticket}; Escape cancels, no UserForm needed) and then re-fetches the full item
'      (GET /items/{id}) once resolved. Only the TARGET FOLDER stays a native InputBox
...
'   5. Finally opens the main (selected) file in Inventor (InvApp.Documents.Open) --
'      component files are left on disk only, not opened as separate documents (Inventor
'      resolves the assembly's own references to them automatically when it opens the
'      assembly).
'
...
' Inventor application object -- declared and assigned at the start of Sub main(), same
' reasoning as EasyPDM.Inventor/EasyPDMUpload.bas ("InvApp" is only guaranteed via an
' explicit GetObject call in an imported module, not automatically visible the way
' "ThisApplication" might be in other contexts). Grouped here with the module's other
' Private/Const declarations, all BEFORE any Sub/Function, matching the same defensive
' layout already applied to EasyPDM.SolidWorks/EasyPDMUpload.bas after a real
' "Variable not defined" lesson learned there.
Private InvApp As Object

' Win32 API used ONLY by WaitForTicket (below) to poll the ticket endpoint while keeping
' Inventor responsive and letting the user cancel with Escape -- see
' EasyPDM.Inventor/EasyPDMUpload.bas's copy of this same block for the full reasoning
' (duplicated here per this file's own self-containment rule, see file header).
#If VBA7 Then
...
Private Function T_PL(ByVal key As String) As String
    Select Case key
        ...
        Case "Dl_WrongFormatSuffix": T_PL = """ (oczekiwano .ipt/.iam) -- pomijam, zeby nie probowac otworzyc niewlasciwego pliku ("
        ...
        Case "Dl_FailedToOpenSuffix": T_PL = " w Inventorze (kod bledu "
...
Private Function T_EN(ByVal key As String) As String
    Select Case key
        ...
        Case "Dl_WrongFormatSuffix": T_EN = """ (expected .ipt/.iam) -- skipping it rather than trying to open the wrong file ("
        ...
        Case "Dl_FailedToOpenSuffix": T_EN = " in Inventor (error code "
...
Private Function T_DE(ByVal key As String) As String
    Select Case key
        ...
        Case "Dl_WrongFormatSuffix": T_DE = """ (erwartet .ipt/.iam) -- wird uebersprungen, statt die falsche Datei zu oeffnen ("
        ...
        Case "Dl_FailedToOpenSuffix": T_DE = " in Inventor fehlgeschlagen (Fehlercode "
...
Function LogFilePath() As String
    LogFilePath = Environ$("TEMP") & "\EasyPDM_inventor_download_macro.log"
End Function
...
' Reads a nested object field (e.g. the "item" key inside a BOM row shaped like
' {"item": {...}, "quantity": ..., "position": ...}) -- Nothing if missing/null.
Function JsonGetObject(ByVal obj As Object, ByVal key As String) As Object
    If obj Is Nothing Then Exit Function
    If Not obj.Exists(key) Then Exit Function
    If IsNull(obj.Item(key)) Then Exit Function
    Set JsonGetObject = obj.Item(key)
End Function
...
Function ApiGet(ByVal path As String) As Object
    ...
    ' practice on the SolidWorks counterpart of this macro: a later GET for the SAME URL
...
' number-entry InputBox. Duplicated from EasyPDM.Inventor/EasyPDMUpload.bas (see file
' header for why this file does not import from it) -- BuildBrowserDownloadUrl differs
' (mode=download, no "name" hint), WaitForTicket/NewGuid/UrlEncode/OpenUrlInBrowser are
' identical copies.
' ============================================================================
...
' times out (10 minutes), or the user presses Escape -- identical to
' EasyPDM.Inventor/EasyPDMUpload.bas's copy, see there for the full reasoning (no UserForm
' in this file, status bar text instead of a dialog, tick/poll counters instead of Timer()
' to avoid a midnight rollover).
'
' UNVERIFIED against a live Inventor install: Application.StatusBarText as a settable
' property (see the two On Error-wrapped assignments below) -- written from documented
' Inventor Automation API behavior, not tested here. Wrapped in On Error Resume Next
' regardless, so a wrong assumption degrades to "no status bar text", not a crash.
...
' ============================================================================
' Login / session -- identical to EasyPDM.Inventor/EasyPDMUpload.bas (including the
' sessionToken-in-JSON-body fix -- MSXML2.XMLHTTP.6.0 does not reliably expose the
' Set-Cookie header).
' ============================================================================
...
' creation. Identical to EasyPDM.SolidWorks/EasyPDMDownload.bas.
' ============================================================================

' Revisions as uppercase letters instead of digits: 1->A, 2->B, ..., 26->Z, 27->AA... --
' the same conversion as in EasyPDMUpload.bas / the web frontend / the other CAD macros.
...
Private Function FindCurrentAttachment(ByVal item As Object, ByVal attachments As Object, Optional ByVal roleFilter As String = "cad") As Object
    ...
    ' on the SolidWorks side, once PDF export was added, since PDF uploads last -- pick a PDF
    ' instead of the real CAD file, causing an error when the macro tried to open it.
...
' ANY Unicode character. Confirmed in practice on the SolidWorks side: VBA's legacy Open
' statement converts the path to the system's current ANSI code page before calling into
' Windows -- an item/attachment name with characters not representable there (e.g. Polish
' diacritics on a non-Polish code page) silently turns into literal "?", which is itself an
' illegal Windows filename character, so the write failed outright with "Bad file name or
' number". ADODB.Stream.SaveToFile goes through the Unicode Windows APIs instead and has no
' such limitation.
...
Function DownloadItem(ByVal item As Object, ByVal targetDir As String) As String
    ...
    ' manually swapped in the web application), refuse to open it as an Inventor document
    ...
    If currentExt <> ".ipt" And currentExt <> ".iam" Then
    ...
    ' alongside the part/assembly file -- so manually opening it later in Inventor resolves
    ' its model reference automatically (same folder, matching filename). Purely best-effort:
    ' a missing/failed drawing download never fails the whole DownloadItem call, since the
    ' part/assembly file above is already saved successfully at this point.
...
Sub main()
    ...
    ' like EasyPDM.FreeCad/EasyPDMDownload.FCMacro and EasyPDM.SolidWorks/
    ' EasyPDMDownload.bas (see BuildBrowserDownloadUrl/WaitForTicket above).
    ...
    If topPath <> "" And Dir(topPath) <> "" Then
        ...
        ' SolidWorks's OpenDoc6, which needs an explicit swDocumentTypes_e value) -- no
        ' extension-to-type mapping needed here. Errors surface as a COM exception rather
        ' than ByRef output parameters, so they are caught via On Error like everywhere else
        ' in this file rather than checked as return values.
        Dim opened As Object
        Dim openErrNum As Long, openErrDesc As String
        On Error Resume Next
        Err.Clear
        Set opened = InvApp.Documents.Open(topPath, True)
        openErrNum = Err.Number
        openErrDesc = Err.Description
        On Error GoTo Failed
        If opened Is Nothing Or openErrNum <> 0 Then
            AppendLog T("Dl_FailedToOpenPrefix") & topPath & T("Dl_FailedToOpenSuffix") & openErrNum & "): " & openErrDesc
        Else
